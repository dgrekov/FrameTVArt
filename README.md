...
| `TOKEN_FILE` | No | `token_file.txt` | Token file stored relative to upstream script directory. Ephemeral unless volume-mounted. |
...
The upstream script stores `uploaded_files.json` and the token file in its own directory. To persist these across container restarts you can mount a volume at `/app/samsung-tv-ws-api/example`:
```bash
docker run -v /host/config:/app/samsung-tv-ws-api/example ...
```
